...
class BuildSiteTests(unittest.TestCase):
    def test_build_site_writes_home_detail_and_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            trend_section = homepage.split('<div class="trend-grid">', 1)[1].split("</section>", 1)[0]
            self.assertIn('<div class="tag">趋势 2</div>', trend_section)
            self.assertIn("第二条", trend_section)
            self.assertNotIn("旧日报变化", trend_section)
            self.assertIn("市场观察", homepage)
