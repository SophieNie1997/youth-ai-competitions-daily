...
MARKET_OBSERVATIONS = [
    ...
    MarketObservation(
        ...
        signal="本表追踪的Samsung中小学组只收香港本地同校团队，且赛事页状态与截止表单不一致；中小学智能未来邀请赛限指定区域学校，重庆机器人竞赛由区县推荐，东博会人形机器人足球则定向南宁10所中学。",
        detail="秋季机会明显分化为公开报名、学校报名、区县推荐和定向邀请。赛事名称里出现“全国”“青少年”或“邀请赛”，都不能替代真实学籍、地域、组队和监护条件。",
        application="咨询入口先设置学校所在地、年级、同校组队、推荐单位和监护同意五项筛选，再展示可执行机会，避免把仅可关注的活动包装成可报名项目。",
    ),
    MarketObservation(
        category="课程设计",
        title="过程证据变成作品的一部分",
        signal="中小学智能未来邀请赛明确要求披露工具、提示词和不少于3次迭代；Samsung Solve for Tomorrow从概念PDF推进到原型、视频、展示和现场Pitch；AI Challenge团队赛还要求可运行网站或移动应用。",
    ...
    MarketObservation(
        ...
        signal="全国通用人工智能挑战赛要求从虚拟仿真迁移到实体车，重庆新赛季覆盖超级轨迹、空地协同救援、ENJOY AI和VEX，南宁校队还要做人形机器人视觉追踪、踢球控制和策略迭代。",
